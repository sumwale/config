...
local function arrange(out)
  ...
  for i = 1, #out do
    ...
    -- add an item for mirror
    local last_item = new[#new]
    if #last_item > 1 then
      new[#new + 1] = gtable.clone(last_item)
      table.insert(new[#new], "--mirror")
    end
...
local function menu()
  ...
  for _, choice in pairs(choices) do
    local cmd = "xrandr"
    local mirror = choice[#choice] == "--mirror"
    if mirror then
      table.remove(choice, #choice)
    end
    -- Enabled outputs
