...
[ "$#" -lt "2" ] && { echo "Usage: `basename "$0"` [-hwaccel|-nohwaccel] [-hevc|-avi] [-iargs <extra ffmpeg args for input>] <input file> [<ffmpeg output options>] <output file>"; exit 1; }
...
# parameters below are taken from the codec guides on https://ffmpeg.party for 30fps
if [ "$1" = "-av1" ]; then
  cv="-c:v libsvtav1 -crf 26 -preset 6 -svtav1-params keyint=10s:tune=0:enable-overlays=1:scd=1:scm=0"
  AV1=1
  shift
elif [ "$1" = "-hevc" ]; then
  cv="-c:v libx265 -crf 25 -x265-params no-open-gop=1:keyint=300:gop-lookahead=12:bframes=6:weightb=1:hme=1:strong-intra-smoothing=0:rect=0:aq-mode=4"
  HEVC=1
  shift
else
  # default to h.264
  cv="-c:v libx264 -crf 21 -tune film -x264-params keyint=300:bframes=6:ref=4:me=umh:subme=9:no-fast-pskip=1:b-adapt=2:aq-mode=2"
fi
...
ffmpeg $hwaccel $input_args -i "$FNAME" -c:a libopus -b:a 128k $cv "$@"
